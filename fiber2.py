...
class Fiber(object):
    ...
    def __init__(self, length = 1, step_length = 1, wavelength = 1.55e-6):
        """
...
class LargeCoreMMF(Fiber):
    ...
    def __init__(self, length = 10.0, step_length = 1.0, frequency = 3e8 / 1.55e-6, **kwargs):
        super(LargeCoreMMF, self).__init__(length, step_length, frequency)
    ...
    def calculate_beta(self, p, q, offset = 0.0):
        """
        ...
        B1 = lambda kappa : self.a * n0xy(kappa)[0]
        B2 = lambda kappa : self.a * n0xy(kappa)[1]
        #A = lambda 
        R=0.0
        alpha = 2.01
        k0 = lambda L : 2 * numpy.pi *(L+offset)/self.c
        D=self.a * self.n0 * sqrt(2.0 * self.DELTA)
        F=gamma(1.0 / alpha + 0.5) * (alpha + 2.0) * (p + q + 1) * sqrt(numpy.pi)
        V = lambda L: 2 * numpy.pi*(L+offset) / self.c * D
        b_tilde = lambda L : pow(F* pow(V(L), 2.0 / alpha), alpha / (2.0 + alpha))
        return (lambda L, kappa : 1.0 *R/ self.a * sqrt(pow(k0(L) * B1(kappa), 2) - pow(b_tilde(L), 2)),
                lambda L, kappa : 1.0 *R/ self.a * sqrt(pow(k0(L) * B2(kappa), 2) - pow(b_tilde(L), 2)))
    ...
    def coupling_coefficients(self, L, kappa, MAX):
        ...
        for m in range(M):
            for n in range(M):
                ...
                if ((pp + 1 == p) or (pp - 1 == p)) and (q == qp):
                    ...
                    C[index_1, index_2] = 2 * numpy.pi* L / self.c * self.n0 * kappa * self.w / 2.0 * \
